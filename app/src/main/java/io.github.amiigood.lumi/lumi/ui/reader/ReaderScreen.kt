...
private fun ImageReader(
    ...
    val webtoonState = rememberLazyListState(state.initialPage)
    ...
    LaunchedEffect(state.mode, pagerState, webtoonState) {
        snapshotFlow {
            when (state.mode) {
                ReadingMode.PAGED -> pagerState.currentPage
                ReadingMode.WEBTOON -> webtoonState.firstVisibleItemIndex
            }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        AnimatedVisibility(
            ...
            val currentPage = when (state.mode) {
                ReadingMode.PAGED -> pagerState.currentPage + 1
                ReadingMode.WEBTOON -> webtoonState.firstVisibleItemIndex + 1
            }
            BottomBar(
...
private fun TopBar(
    ...
    Row(
        ...
        IconButton(onClick = onToggleMode) {
            Icon(
                imageVector = if (mode == ReadingMode.PAGED) {
